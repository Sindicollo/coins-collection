...
export function CoinCard({ coin, onEdit, onDelete, onSelect }: CoinCardProps): React.ReactElement {
  ...
  // Retrieve cached photo list and data URLs synchronously (O(1) Map lookups).
  // This lets us show the correct number of skeleton squares on repeat visits,
  // or skip the loading state entirely when everything is cached.
  ...
  // -1 = unknown, 0–4 = known (capped at 4 because we only show 4 thumbnails)
  ...
  // Load photo thumbnails (skip when everything is already cached)
  React.useEffect(() => {
  ...
  return (
    <Card
      className={`p-3 cursor-pointer hover:shadow-md transition-shadow group relative ${
        coin.onAuction && !coin.sold ? 'bg-orange-50 border-orange-800' : ''
      }`}
    ...
    >
      ...
      <div className={`${coin.sold && !coin.onAuction ? 'opacity-60' : ''}`}>
        ...
        <div className="h-16 flex items-center gap-1.5">
          {!photosLoaded ? (
            /* Loading skeleton — count matches actual photos when cached */
            <>
              {knownCount >= 0
                ...
                : (
                    /* Unknown count (first ever visit) — single placeholder bar */
